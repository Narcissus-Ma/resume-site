    ...
    >
      <div className="container mx-auto px-4">
        <div className="text-center mb-16">
          ...
          <Paragraph
            className={`mt-4 max-w-2xl mx-auto ${darkMode ? 'text-gray-400' : 'text-gray-600'}`}
          >
        ...
        <Row gutter={[24, 24]}>
          {skills.map((skill, index) => (
            <Col key={index} md={12} xs={24}>
              <Card
                className={`border-0 shadow-md hover:shadow-lg transition-shadow duration-300 ${darkMode ? 'bg-gray-800' : ''}`}
              >
                <div className="p-6">
                  <div className="flex justify-between items-center mb-2">
                    <Text strong className={`text-lg ${darkMode ? 'text-white' : 'text-gray-800'}`}>
                      {skill.name}
                    </Text>
                    <Text className={darkMode ? 'text-gray-300' : ''} type="secondary">
                      {skill.value}%
                    </Text>
                  </div>
                  <Progress
        ...
        <Row className="mt-12" gutter={[24, 24]}>
          <Col md={8} xs={24}>
            <Card
              className={`border-0 shadow-md hover:shadow-lg transition-all duration-300 hover:-translate-y-1 ${darkMode ? 'bg-gray-800' : ''}`}
            >
              <Meta
