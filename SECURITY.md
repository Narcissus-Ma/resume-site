# 安全政策 / Security Policy

## 支持范围 / Supported Versions

当前仅支持 `main` 分支上的最新代码。请先确认问题是否仍能在最新 `main` 上复现。

## 报告漏洞 / Reporting a Vulnerability

请勿在公开 Issue、讨论区或 Pull Request 中披露凭据、私密配置、漏洞利用代码、完整攻击路径或其他可能帮助攻击者的信息。

请通过私下渠道联系 `SECURITY_CONTACT` 报告安全问题。发布仓库前，维护者必须将该占位符替换为实际且可访问的安全联系邮箱、私信渠道或漏洞报告入口。

报告中建议包含：

- 受影响的版本或提交；
- 问题影响与复现所需的最少信息；
- 已脱敏的复现步骤或概念验证；
- 可能的缓解建议（如有）。

## 处理流程 / Process

维护者会确认收到报告、评估影响并在可行时制定修复方案。修复完成后，维护者会与报告者协调披露方式，并在适当的情况下发布修复说明或致谢。处理优先级和时间取决于影响范围、复现条件及维护者可用性，因此不承诺固定响应或修复时限。
